...
- The plugin defines its own BaoTa-documented POST transport to `/plugin?action=a&s=<action>&name=<plugin>`, so it does not depend on a host-global helper. Local static previews still cannot call panel endpoints and must report that limitation without exposing any credential.
